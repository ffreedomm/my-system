    ...
    <div>
        <div class="container">
            <div class="handle-box">
                <el-input v-model="name" placeholder="行业名称" class="handle-input mr10"></el-input>
...
<script>
import { queryTradeList,queryTradeListSum,addTrade,updateTrade,removeTrade,orgListForTrade,orgListSumForTrade } from '@/api/baseInfo'
export default {
    name: 'industry',
    data() {
        return {
    ...
    methods: {
        getData() {
            queryTradeList(this.name, this.start, this.end).then(res => {
              if(res.success){
       ...
       // 下属企业详情操作
        handleDetail(row) {
          orgListForTrade(row.id, this.startOrg, this.endOrg).then(res=>{
             if(res.success){
                this.orgData = res.object
                this.orgListSumForTrade(row)
